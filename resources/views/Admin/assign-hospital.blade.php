			...
			<div class="page-content">
						...
						<div class="card border-top border-0 border-4 border-danger">
							<div class="card-body">
								<div class="card-title d-flex align-items-center">
									<h5 class="mb-0 text-danger">Assign Hospital</h5>
								</div>
								<hr>
								<form class="row g-3" action="{{route('panel.User.assign-hospital')}}" method="POST" enctype="multipart/form-data">
								@csrf
				...
				<div class="card border-top border-0 border-4 border-danger">
                    ...
                    <th style="font-size:14px;">Date</th>
								...
								<tbody>
                                @php 
                                $district331 = DB::table('assign_hospital')->where('staff_id',$id)->orderBy('id','DESC')->get();
                                @endphp 

                                @foreach($district331 as $district331)

                                @php 
                                $district33 = DB::table('add_hospital')->where('hospital_id',$district331->hospital_id)->first();
                                @endphp 
                                    <tr>
                                    <td>{{ $loop->iteration }}</td> 
                                    <td>
                                        <div class="action">
                                        <a onclick="status_change_alert('{{route('panel.User.delete-hospital',[$district331->id])}}','{{('Want to delete this Hospital ?')}}',event)" class="btn btn-danger" style="padding: 23px 7px 8px;" title="Delete">
