@extends('Admin.app')
@section('title',('Today Submitted Report'))
...
<div class="page-wrapper">
			<div class="page-content">
				<h6 class="mb-0 text-uppercase">Today Submitted Report</h6>
				<hr/>
				<div class="card border-top border-0 border-4 border-danger">
                    ...
                    <tr>
                      <td style="padding: 28px 11px 0px;">
                        <!-- <a class="row g-3" data-bs-toggle="modal" data-bs-target="#exampleModal{{$data->hospital_id}}" style=" cursor: pointer;background: #e0efff; padding: 0px 10px 20px 10px; border-radius: 8px;"> -->
                        <a class="row g-3" href="{{route('panel.TodayReport.today-submitted-report-sample',[$data->hospital_id])}}" style=" cursor: pointer;background: #e0efff; padding: 0px 10px 20px 10px; border-radius: 8px;">
                      ...
                      <!-- <div class="modal fade" id="exampleModal{{$data->hospital_id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog">
                          <div class="modal-content">
                            ...
                            @php 
                            $category = DB::table('add_submitted_hospital_sample')->leftjoin('add_hospital','add_submitted_hospital_sample.hospital_id','add_hospital.hospital_id')->leftjoin('add_sample_box_detail','add_submitted_hospital_sample.sample_selected_id','add_sample_box_detail.id')->where('add_submitted_hospital_sample.hospital_id',$data->hospital_id)->where('add_submitted_hospital_sample.created',date('Y-m-d'))->orderBy('add_submitted_hospital_sample.id','DESC')->select('add_submitted_hospital_sample.sample_selected_id','add_sample_box_detail.nikshay_id','add_sample_box_detail.patient','add_sample_box_detail.invoice_photo')->get();
                            @endphp
                            @foreach($category as $val3)
                              <label class="btn btn-danger" style="color:white; margin-bottom:10px; width:100%;text-align: left; cursor:initial;">{{$val3->nikshay_id}} - {{$val3->patient}} <a href="{{route('panel.Invoice.sample-submitted-info',[$val3->sample_selected_id])}}"  target="_blank" style="padding: 5px 10px; font-size: 12px; color: #fb3f60; background: white; float: right; border-radius: 3px; font-weight: 500;">View Info</a>
                                          </label><br>
                            @endforeach
                            </div>
